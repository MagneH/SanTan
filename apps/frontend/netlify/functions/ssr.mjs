// Import server entry from Nitro output
import { default as nitroHandler } from '../../.output/server/index.mjs'

if (!nitroHandler || typeof nitroHandler !== 'function') {
  console.error('[netlify ssr] Nitro handler not found in .output/server/index.mjs')
}
...
export async function handler(event) {
  if (!nitroHandler) {
    return {
      ...
      body: 'SSR handler missing - Nitro build output not found'
    }
  }

  try {
    const req = buildRequest(event)
    const res = await nitroHandler(req)
